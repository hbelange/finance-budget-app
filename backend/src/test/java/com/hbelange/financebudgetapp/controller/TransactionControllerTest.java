...
class TransactionControllerTest {
    ...
    void createTransaction_returns201WithDto() throws Exception {
        when(transactionService.create(any(), any())).thenReturn(SAMPLE_DTO);

        mockMvc.perform(post("/api/transactions").with(jwt())
    ...
    void updateTransaction_returns200WithDto() throws Exception {
        when(transactionService.update(eq(TRANSACTION_ID), any(), any())).thenReturn(SAMPLE_DTO);

        mockMvc.perform(put("/api/transactions/" + TRANSACTION_ID).with(jwt())
                .contentType(MediaType.APPLICATION_JSON)
                .content("""
                    {
                        "accountId": "%s",
    ...
    void updateTransaction_returns404_whenTransactionNotFound() throws Exception {
        when(transactionService.update(eq(TRANSACTION_ID), any(), any()))
    ...
    void getTransactions_returns200_withNoFilters() throws Exception {
        when(transactionService.findAll(isNull(), isNull(), any(), any(Pageable.class)))
            .thenReturn(new PageImpl<>(List.of(SAMPLE_DTO)));

        mockMvc.perform(get("/api/transactions").with(jwt()))
            .andExpect(status().isOk())
    ...
    void getTransactions_returns200_withAccountIdFilter() throws Exception {
        when(transactionService.findAll(eq(ACCOUNT_ID), isNull(), any(), any(Pageable.class)))
            .thenReturn(new PageImpl<>(List.of(SAMPLE_DTO)));
    ...
    void getTransactions_returns200_withMonthFilter() throws Exception {
        when(transactionService.findAll(isNull(), eq(YearMonth.of(2026, 1)), any(), any(Pageable.class)))
            .thenReturn(new PageImpl<>(List.of(SAMPLE_DTO)));
    ...
    void getTransactions_returns200_withAccountIdAndMonthFilter() throws Exception {
        when(transactionService.findAll(eq(ACCOUNT_ID), eq(YearMonth.of(2026, 1)), any(), any(Pageable.class)))
            .thenReturn(new PageImpl<>(List.of(SAMPLE_DTO)));
